...
module ndbservices {
    ...
    export interface IDataServiceResource<T extends ng.resource.IResource<T>> extends ng.resource.IResourceClass<T> {
    }
    
    export interface ApiResourceItem<T> extends ng.resource.IResource<T>, IApiItem {
        
    }

    export abstract class DataService<T extends ApiResourceItem<T>> {
        ...
        public refreshData() : Promise<boolean> {
            ...
            return new Promise<boolean>((resolve, reject) => {
                this.dataSource.query((data, headers) => {
                    data = data.map((obj) => {
                        return this.mapQueriedItem(obj);
                    });
